...
async function $do(client, request, options) {
    const parsed = (0, schemas_js_1.safeParse)(request, (value) => z.parse(orderslist_js_1.OrdersListRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
        return [(0, operations_js_1.haltIterator)(parsed), { status: "invalid" }];
    }
    const payload = parsed.value;
    const body = null;
    const path = (0, url_js_1.pathToFunc)("/v1/orders/")();
    ...
    const [result, raw] = await M.match(M.json(200, orderslist_js_1.OrdersListResponse$inboundSchema, { key: "Result" }), M.jsonErr(422, httpvalidationerror_js_1.HTTPValidationError$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, req, { extraFields: responseFields });
    if (!result.ok) {
