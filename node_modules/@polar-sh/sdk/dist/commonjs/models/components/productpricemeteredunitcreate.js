"use strict";
/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.ProductPriceMeteredUnitCreate$outboundSchema = exports.UnitAmount$outboundSchema = void 0;
exports.unitAmountToJSON = unitAmountToJSON;
exports.productPriceMeteredUnitCreateToJSON = productPriceMeteredUnitCreateToJSON;
const z = __importStar(require("zod/v4-mini"));
const primitives_js_1 = require("../../lib/primitives.js");
const smartUnion_js_1 = require("../../types/smartUnion.js");
const presentmentcurrency_js_1 = require("./presentmentcurrency.js");
const taxbehavioroption_js_1 = require("./taxbehavioroption.js");
/** @internal */
exports.UnitAmount$outboundSchema = (0, smartUnion_js_1.smartUnion)([z.number(), z.string()]);
function unitAmountToJSON(unitAmount) {
    return JSON.stringify(exports.UnitAmount$outboundSchema.parse(unitAmount));
}
/** @internal */
exports.ProductPriceMeteredUnitCreate$outboundSchema = z.pipe(z.object({
    amountType: z.literal("metered_unit"),
    priceCurrency: z.optional(presentmentcurrency_js_1.PresentmentCurrency$outboundSchema),
    taxBehavior: z.optional(z.nullable(taxbehavioroption_js_1.TaxBehaviorOption$outboundSchema)),
    meterId: z.string(),
    unitAmount: (0, smartUnion_js_1.smartUnion)([z.number(), z.string()]),
    capAmount: z.optional(z.nullable(z.int())),
}), z.transform((v) => {
    return (0, primitives_js_1.remap)(v, {
        amountType: "amount_type",
        priceCurrency: "price_currency",
        taxBehavior: "tax_behavior",
        meterId: "meter_id",
        unitAmount: "unit_amount",
        capAmount: "cap_amount",
    });
}));
function productPriceMeteredUnitCreateToJSON(productPriceMeteredUnitCreate) {
    return JSON.stringify(exports.ProductPriceMeteredUnitCreate$outboundSchema.parse(productPriceMeteredUnitCreate));
}
//# sourceMappingURL=productpricemeteredunitcreate.js.map