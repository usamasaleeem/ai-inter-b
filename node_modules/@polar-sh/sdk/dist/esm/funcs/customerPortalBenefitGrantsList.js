...
import { CustomerPortalBenefitGrantsListRequest$outboundSchema, CustomerPortalBenefitGrantsListResponse$inboundSchema, } from "../models/operations/customerportalbenefitgrantslist.js";
 ...
 */
export function customerPortalBenefitGrantsList(client, security, request, options) {
    return new APIPromise($do(client, security, request, options));
}
async function $do(client, security, request, options) {
    const parsed = safeParse(request, (value) => z.parse(CustomerPortalBenefitGrantsListRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result, raw] = await M.match(M.json(200, CustomerPortalBenefitGrantsListResponse$inboundSchema, {
        key: "Result",
