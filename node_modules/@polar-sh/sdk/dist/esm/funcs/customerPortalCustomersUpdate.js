...
import { CustomerPortalCustomerUpdate$outboundSchema, } from "../models/components/customerportalcustomerupdate.js";
 ...
 */
export function customerPortalCustomersUpdate(client, security, request, options) {
    return new APIPromise($do(client, security, request, options));
}
async function $do(client, security, request, options) {
    const parsed = safeParse(request, (value) => z.parse(CustomerPortalCustomerUpdate$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result] = await M.match(M.json(200, CustomerPortalCustomer$inboundSchema), M.jsonErr(422, HTTPValidationError$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, req, { extraFields: responseFields });
    if (!result.ok) {
