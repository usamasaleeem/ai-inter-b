...
import { Unauthorized$inboundSchema, } from "../models/errors/unauthorized.js";
import { CustomerPortalLicenseKeysListRequest$outboundSchema, CustomerPortalLicenseKeysListResponse$inboundSchema, } from "../models/operations/customerportallicensekeyslist.js";
 ...
 */
export function customerPortalLicenseKeysList(client, security, request, options) {
    return new APIPromise($do(client, security, request, options));
}
async function $do(client, security, request, options) {
    const parsed = safeParse(request, (value) => z.parse(CustomerPortalLicenseKeysListRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result, raw] = await M.match(M.json(200, CustomerPortalLicenseKeysListResponse$inboundSchema, {
        key: "Result",
    }), M.jsonErr(401, Unauthorized$inboundSchema), M.jsonErr(404, ResourceNotFound$inboundSchema), M.jsonErr(422, HTTPValidationError$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, req, { extraFields: responseFields });
    if (!result.ok) {
