...
import { CustomerPortalOrdersListRequest$outboundSchema, CustomerPortalOrdersListResponse$inboundSchema, } from "../models/operations/customerportalorderslist.js";
 ...
 */
export function customerPortalOrdersList(client, security, request, options) {
    return new APIPromise($do(client, security, request, options));
}
async function $do(client, security, request, options) {
    const parsed = safeParse(request, (value) => z.parse(CustomerPortalOrdersListRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result, raw] = await M.match(M.json(200, CustomerPortalOrdersListResponse$inboundSchema, {
        key: "Result",
