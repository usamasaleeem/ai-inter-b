...
import { CustomerPortalSubscriptionsListRequest$outboundSchema, CustomerPortalSubscriptionsListResponse$inboundSchema, } from "../models/operations/customerportalsubscriptionslist.js";
 ...
 */
export function customerPortalSubscriptionsList(client, security, request, options) {
    return new APIPromise($do(client, security, request, options));
}
async function $do(client, security, request, options) {
    const parsed = safeParse(request, (value) => z.parse(CustomerPortalSubscriptionsListRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result, raw] = await M.match(M.json(200, CustomerPortalSubscriptionsListResponse$inboundSchema, {
        key: "Result",
