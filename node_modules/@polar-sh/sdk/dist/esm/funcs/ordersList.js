...
import { OrdersListRequest$outboundSchema, OrdersListResponse$inboundSchema, } from "../models/operations/orderslist.js";
 ...
 * **Scopes**: `orders:read`
 */
export function ordersList(client, request, options) {
    return new APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = safeParse(request, (value) => z.parse(OrdersListRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result, raw] = await M.match(M.json(200, OrdersListResponse$inboundSchema, { key: "Result" }), M.jsonErr(422, HTTPValidationError$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, req, { extraFields: responseFields });
    if (!result.ok) {
