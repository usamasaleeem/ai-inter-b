...
import { SubscriptionsListRequest$outboundSchema, SubscriptionsListResponse$inboundSchema, } from "../models/operations/subscriptionslist.js";
 ...
 */
export function subscriptionsList(client, request, options) {
    return new APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = safeParse(request, (value) => z.parse(SubscriptionsListRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result, raw] = await M.match(M.json(200, SubscriptionsListResponse$inboundSchema, { key: "Result" }), M.jsonErr(422, HTTPValidationError$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, req, { extraFields: responseFields });
    if (!result.ok) {
